...
def save_training_data(extent, data, name, x_label="Position", y_label="Velocity",
                       ticks=None, xticklabels=None, yticklabels=None,
...
def save_policy(X, V, data, name, x_label="Position", y_label="Velocity", cmap='PRGn', norm_gamma=1,
...
def save_value(X, V, data, name, x_label="Position", y_label="Velocity", ticks=None, cmap='magma', norm_gamma=1,
               format="pdf", dpi=600):
    plt.figure()
...
def plot(X, V, data, x_label="Position", y_label="Velocity", cmap='viridis'):
...
def save_accuracy_curve(x, mean, std, reference=None, x_label="Number of samples",
    ...
    plt.xlabel(x_label);
    plt.ylabel("Accuracy of solution (L2)")
    plt.savefig(f"images/{name}.{format}", dpi=dpi);
    plt.close()

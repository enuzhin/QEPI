...
def discretize(state):
    ...
    return ((x_l.numpy(),v_l.numpy()),(x_u.numpy(),v_u.numpy())),((p_x_l.numpy(),p_v_l.numpy()),(p_x_u.numpy(), p_v_u.numpy()))
...
def to_sle(discrete_state_new,not_done,r):
    ...
    P = (gamma * (T_ll + T_lu + T_ul + T_uu) * not_done[np.newaxis,np.newaxis,:,:]- np.eye(Nx)[:,np.newaxis,:,np.newaxis]*np.eye(Nv)[np.newaxis,:,np.newaxis,:]).reshape(Nx*Nv,Nx*Nv)
    r = r.reshape(-1)
